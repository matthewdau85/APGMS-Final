...
async function main() {
    ...
    app.get("/balances/:code", async (request, reply) => {
...
async function publishSyntheticPayroll(bus, subject, orgId) {
    const envelope = {
        id: uuidv7(),
        orgId,
...
async function ensureBaselineAccounts(orgId) {
    const codes = [
    ...
    const records = await Promise.all(codes.map(async (entry) => prisma.account.upsert({
