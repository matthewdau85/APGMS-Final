 ...
 * Unit tests must satisfy that guard, otherwise you'll get 403 and the
 * assertions will read undefined fields from the error body.
 *
 * We include multiple headers to match current/legacy guard behaviors.
 */
const PROTOTYPE_ADMIN_TOKEN = process.env.PROTOTYPE_ADMIN_TOKEN ?? "admin-token";

function prototypeAdminHeaders(): Record<string, string> {
  return {
    // common pattern
    authorization: `Bearer ${PROTOTYPE_ADMIN_TOKEN}`,

    // explicit prototype flag(s)
    "x-prototype-admin": "true",
    "x-prototype-admin-token": PROTOTYPE_ADMIN_TOKEN,

    // legacy/admin-role variants (harmless if ignored)
    "x-admin": "true",
    "x-role": "admin",
    "x-user-role": "admin",

    // audit-ish context used elsewhere
    "x-actor": "admin-1",
  };
}
...
describe("regulator compliance summary route", () => {
  beforeAll(() => {
    // If the guard reads from env, make it deterministic for unit tests.
    process.env.PROTOTYPE_ADMIN_TOKEN = PROTOTYPE_ADMIN_TOKEN;
  });
