...
import { startRiskEventIngestionWorker } from "./ingestion/risk-event-worker.js";

export { runNightlyDesignatedAccountReconciliation } from "./jobs/designated-reconciliation.js";
export { startRiskEventIngestionWorker } from "./ingestion/risk-event-worker.js";

const modulePath = fileURLToPath(import.meta.url);
const invokedPath = process.argv[1] ? resolve(process.argv[1]) : null;

if (invokedPath && resolve(modulePath) === invokedPath) {
  const mode = process.argv[2] ?? "reconciliation";
  if (mode === "risk-ingestor") {
    startRiskEventIngestionWorker()
      .catch((error) => {
        console.error("Risk event ingestion worker failed", error);
        process.exitCode = 1;
      });
  } else {
