...
type InMemoryPrisma = InMemoryTransaction & {
  $transaction: <T>(fn: (tx: InMemoryTransaction) => Promise<T>) => Promise<T>;
};
...
test("feature flag disabled rejects operations", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.equal((error as any).code, "detector_muting_disabled");
      return true;
    },
  );
});
