...
- **PayTo + Multi-bank enforcement**: the payments service (`services/payments/src`) ships with a `PayToService` abstraction and mock providers for CBA, NAB, and ANZ. Each `DesignatedAccount` stores its PayTo mandate ID and current status so the worker can block transfers until the status flips to `ACTIVE`. NAB/ANZ accounts are forced into deposit-only mode and any withdrawal attempt will throw `deposit-only` errors, which our jest suite covers via `services/payments/test/payto.test.ts`.
- **Runbook for creating mandated accounts**:
  1. Instantiate `MultiBankPayToService(createDefaultBankProviders())` and `DesignatedAccountOrchestrator` inside an admin shell (see `services/payments/src/index.ts`).
  2. Call `createAccount` for each obligation (PAYGW, GST, PAYGI). The orchestration layer writes the newly generated `payToMandateId` back onto the designated account record immediately so it can be audited.
  3. Drive `activateMandate(accountId)` once the customer authorises the PayTo request. This will call `authorizeMandate` + `verifyMandate` against the mocked bank API before flipping the stored status to `ACTIVE`.
  4. Only after step 3 should `initiateTransfer(accountId, amount, 'DEPOSIT')` be invoked; the method raises `Transfers are blocked...` if the mandate has not been verified, ensuring we never move funds to an unverified destination.
  5. Document each activation outcome in `artifacts/compliance/payto/<org>.json` and include NAB/ANZ deposit-only confirmations whenever you onboard a new obligation-specific account.
