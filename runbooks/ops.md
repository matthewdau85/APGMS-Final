...
- `/compliance/precheck`, `/compliance/status`, and `/compliance/pending` expose readiness, buffer snapshots, pending contributions, and guidance for remediation. Use these endpoints to re-ingest missing batches, rerun the reconciliation job, and capture the proof in the audit log before notifying the business or the ATO.
- Document compliance notification/remediation timelines on the dashboard (pending contributions, active alerts, penalties/remissions, payment-plan status). Capture the adapter provider (via `configureBankingAdapter`) and shortfall resolution steps in `artifacts/compliance/` so the ATO can trace your evidence. Automated reminders should flag upcoming BAS deadlines and penalties while the system records any remission efforts.
