...
| --- | --- | --- |
| Provider abstraction | Every banking adapter (`providers/banking/*`) implements the shared interface, respects per-provider rate caps, and can register without downstream changes. | Audit `providers/banking` and the connector wiring for caps/ratios, add a regression test that exercises a mock provider to prove the shared surface is reusable, and log/document the per-adapter `maxWriteCents` guards so regulators can confirm the caps are enforced. |
