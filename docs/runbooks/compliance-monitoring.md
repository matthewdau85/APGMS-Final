...
- Use `/admin/tier-tuning` to adjust the coverage margin (default 10%) and the cadence (default every 24h) before pointing cron at `/compliance/tier-check`; the API writes `artifacts/compliance/tier-tuning.json` and the schedule metadata returned from `/compliance/status` keeps you honest about the next run time.
