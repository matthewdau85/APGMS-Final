...
import { assertDesignatedAccountMovementAllowed, assertDesignatedAccountTypeIsSupported, DesignatedAccountOperation, DesignatedAccountRuleError, isActiveForNewDesignations, } from "./guards";
...
describe("assertDesignatedAccountMovementAllowed", () => {
    it("allows deposits into ACTIVE accounts", () => {
        ...
        expect(() => assertDesignatedAccountMovementAllowed({
            account,
...
describe("isActiveForNewDesignations", () => {
    it("returns true only for ACTIVE accounts", () => {
        expect(isActiveForNewDesignations(makeAccount({
            lifecycle: DesignatedAccountLifecycle.ACTIVE,
        }))).toBe(true);
        expect(isActiveForNewDesignations(makeAccount({
            lifecycle: DesignatedAccountLifecycle.PENDING_ACTIVATION,
...
describe("assertDesignatedAccountTypeIsSupported", () => {
    it("accepts all known AU tax account types", () => {
        ...
        for (const type of lifecycles) {
            ...
            expect(() => assertDesignatedAccountTypeIsSupported(account)).not.toThrow();
        }
    });
    it("rejects unsupported account types", () => {
        ...
        expect(() => assertDesignatedAccountTypeIsSupported(bogusAccount)).toThrow(DesignatedAccountRuleError);
    });
});
