...
async function verifyRequestImpl(
...
export async function verifyRequest(
  ...
  return verifyRequestImpl(request, reply);
}
...
export function requireRole(
  ...
  const hasRole = principal.roles.some((role) =>
    allowed.some((allowedRole) => {
      const actual = Buffer.from(role);
      const target = Buffer.from(allowedRole);
...
export async function authenticateRequest(
  ...
  try {
    const principal = await authInternals.verifyRequest(request, reply);
